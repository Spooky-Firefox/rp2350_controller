...
type UsMeasure0 = rp2350_controller::hc_rs04::HcSr04Measure<'static, UsTrig0, hal::pwm::Pwm7>;
type UsMeasure1 = rp2350_controller::hc_rs04::HcSr04Measure<'static, UsTrig1, hal::pwm::Pwm6>;
type UsMeasure2 = rp2350_controller::hc_rs04::HcSr04Measure<'static, UsTrig2, hal::pwm::Pwm5>;
type UsOnIrq0 =
    rp2350_controller::hc_rs04::HcSr04OnInterrupt<'static, gpio::bank0::Gpio15, gpio::PullDown>;
type UsOnIrq1 =
    rp2350_controller::hc_rs04::HcSr04OnInterrupt<'static, gpio::bank0::Gpio13, gpio::PullDown>;
type UsOnIrq2 =
    rp2350_controller::hc_rs04::HcSr04OnInterrupt<'static, gpio::bank0::Gpio11, gpio::PullDown>;
...
mod app {
    ...
    #[init(local = [
        ...
        us0_shared: hc_rs04::HcSr04Shared = hc_rs04::HcSr04Shared::new(),
        us1_shared: hc_rs04::HcSr04Shared = hc_rs04::HcSr04Shared::new(),
        us2_shared: hc_rs04::HcSr04Shared = hc_rs04::HcSr04Shared::new(),
    ])]
    fn init(ctx: init::Context) -> (Shared, Local) {
        ...
        // Each PWM slice runs at 1 MHz (div_int=150 @ 150 MHz), so 1 tick = 1 us.
    ...
    #[task(binds = IO_IRQ_BANK0, local = [cnt: u32 = 0,encoder, encoder_last_time, us0_irq, us1_irq, us2_irq], shared = [speed_setpoint_mps, measured_speed_mps, last_sensor_irq_us, fifo], priority = 3)]
    fn gpio_interrupt(mut ctx: gpio_interrupt::Context) {
        ...
        *ctx.local.cnt += 1;
        ...
        // Forward ultrasonic echo-fall interrupts to the sensor drivers.
    ...
    #[task(local = [us0, us1, us2], shared = [speed_setpoint_mps, fifo], priority = 1)]
    async fn ultrasound_scan(mut ctx: ultrasound_scan::Context) -> ! {
        const TIMEOUT_US: u64 = 50_000;
        const SCAN_PERIOD_US: u64 = 60_000;

        let mut next_scan = MainMono::now();

        loop {
            let left_cm = match ctx.local.us0.measure::<MainMono>(TIMEOUT_US.micros()).await {
                Ok(ticks) => ticks as f32 / 58.0,
                Err(_) => {
                    warn!("us0: timeout");
                    f32::INFINITY
                }
            };
            let center_cm = match ctx.local.us1.measure::<MainMono>(TIMEOUT_US.micros()).await {
                Ok(ticks) => ticks as f32 / 58.0,
                Err(_) => {
                    warn!("us1: timeout");
                    f32::INFINITY
            ...
            let right_cm = match ctx.local.us2.measure::<MainMono>(TIMEOUT_US.micros()).await {
                Ok(ticks) => ticks as f32 / 58.0,
                Err(_) => {
                    warn!("us2: timeout");
                    f32::INFINITY
            ...
            let timestamp_us = MainMono::now().ticks();
            let setpoint_mps = ctx.shared.speed_setpoint_mps.lock(|setpoint| *setpoint);
            ...
            debug!(
                "ultrasound: L={} cm C={} cm R={} cm",
                left_cm,
                center_cm,
                right_cm
            );

            next_scan += SCAN_PERIOD_US.micros();
            MainMono::delay_until(next_scan).await;
        }
    }
